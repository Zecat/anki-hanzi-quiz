...
/* the special entry '_variables' in prefixes gives directly the variable value instead of the path*/
  if (prefixes && prefixes._variables){// TODO cleanup and use symbol
    const tmp = get_prop(prefixes._variables, path)
    if (tmp != invalidPathSymbol)
      return tmp
  }
...
export const bracketEval = (query, scope, prefixes, onChange) => {
  ...
  if (bindArgsStr == undefined){
    ...
    let initialUseAttr = getScopedProp(path, scope, prefixes)
